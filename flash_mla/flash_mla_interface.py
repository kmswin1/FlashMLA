...
def flash_mla_sparse_bwd(
    ...
    See design_notes/m16_sparse_bwd_design.md in the Megatron-LM repo for the
    full contract. The LSE handed in must be base-2 (matching FlashMLA fwd's
    output and Megatron's DSA convention) — multiply external base-e LSEs by
    log2(e) before calling.

    M4.14d: dq_acc_workspace and dkv_acc_workspace are caller-allocated
    persistent fp32 buffers (mirrors dense FMHA BWD workspace pattern). The
    kernel memsets and accumulates into these but does NOT manage their
    lifetime. Reuse a single buffer pair across all BWD calls — Megatron's
    sparse_mla_bwd_interface in dsa_fused_kernels.py keeps them at module
    level. Sizes:
    ...
    Args:
        ...
        fuse_reducesum: bool                     (also emit kl_target for the
                                                  ...
                                                  retires the separate
                                                  sparse_mla_topk_reducesum kernel)
