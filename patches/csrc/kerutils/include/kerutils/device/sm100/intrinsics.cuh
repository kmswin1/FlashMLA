...
// barrier instead of a custom transac_bar_t. M16 v2 sparse bwd uses this so the
...
// semantics is single-CTA. Sparse FWD uses cta_group::2 (cluster<2>), v1 used
// our explicit cta_group::1 (unverified SM100 support for gather4). With no
// qualifier we get plain single-CTA gather, which should be the safest.
